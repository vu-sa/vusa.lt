...
describe('threaded creation via HasComments::comment', function (): void {
    test('root, reply, and nested reply compute thread_root_id', function (): void {
        ...
        expect($reply->parent_id)->toBe($root->id)
            ->and($reply->thread_root_id)->toBe($root->id);
        ...
        expect($nested->parent_id)->toBe($reply->id)
            ->and($nested->thread_root_id)->toBe($root->id);
    });

    test('creating a comment dispatches CommentPosted', function (): void {
    ...
    test('mentions are extracted from the body on create', function (): void {
        $this->actingAs($this->author);
    ...
    test('rootComments returns only thread roots', function (): void {
        $this->actingAs($this->author);
        ...
        expect($this->agendaItem->comments()->count())->toBe(2)
            ->and($this->agendaItem->rootComments()->count())->toBe(1);
    });
});

describe('extractMentions', function (): void {
    test('parses unique data-id values', function (): void {
    ...
    test('returns empty array when there are no mentions', function (): void {
        expect(Comment::extractMentions('<p>plain</p>'))->toBeEmpty();
    });
});

describe('body sanitization', function (): void {
    test('strips script tags on create', function (): void {
        $this->actingAs($this->author);
    ...
    test('strips event handler attributes and disallowed tags', function (): void {
        $this->actingAs($this->author);
    ...
    test('keeps allowed formatting and mention markup', function (): void {
        $this->actingAs($this->author);
        $mentioned = User::factory()->create();
    ...
    test('neutralizes javascript link schemes', function (): void {
        $this->actingAs($this->author);
    ...
    test('sanitizes on direct body assignment', function (): void {
        $this->actingAs($this->author);
...
describe('resolve helpers', function (): void {
    test('resolve and unresolve toggle the resolved state', function (): void {
        $this->actingAs($this->author);
        ...
        expect($comment->fresh()->isResolved())->toBeTrue()
            ->and($comment->fresh()->resolved_by)->toBe($resolver->id);

        $comment->unresolve();
        expect($comment->fresh()->isResolved())->toBeFalse()
            ->and($comment->fresh()->resolved_by)->toBeNull();
    });
});

describe('reactions', function (): void {
    test('a comment has many reactions and enforces one emoji per user', function (): void {
        $this->actingAs($this->author);
