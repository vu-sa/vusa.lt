...
test('creates new content parts in order', function () {
...
test('updates an existing part in place', function () {
...
test('deletes parts that are no longer present in the payload', function () {
    $this->content->parts()->create(['type' => 'tiptap', 'json_content' => ['type' => 'doc'], 'order' => 0]);
...
test('rejects an invalid type when creating a part', function () {
    Log::spy();
    ...
    expect($this->content->fresh('parts')->parts)->toHaveCount(0);
    Log::shouldHaveReceived('warning')->once();
});

test('rejects an invalid type when updating an existing part (regression)', function () {
...
test('round-trips options.width', function () {
    $this->service->updateContentParts($this->content, [
...
test('ignores null entries in the payload', function () {
    $this->service->updateContentParts($this->content, [
        null,
        ['type' => 'tiptap', 'json_content' => ['type' => 'doc'], 'options' => null],
    ]);
