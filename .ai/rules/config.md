...
- `app/Settings/*` (spatie/laravel-settings) — anything the board should change themselves. `SiteSettings::$privacy_page_id_lt` is the pattern: store a record **id**, resolve the URL server-side, and let the consumer hide the link when it is null.
