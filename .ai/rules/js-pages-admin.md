...
- Budgets: uppercase for page titles (`u-display`, `text-4xl`/`5xl` on collections, up to `5xl` on records), eyebrows, workspace tabs and button labels — the button voice lives in `ui/button`, don't restyle it per page. One brand-filled action per region. Minimum 12px text (11px eyebrows and table header labels); section headings are sentence case.
