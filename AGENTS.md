...
5. **Budgets** — one brand fill per region; uppercase for display titles, eyebrows, tabs and button labels (the `ui/button` voice), sentence case everywhere else; status colours are never brand.
