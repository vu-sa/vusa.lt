<template>
  <PageContent :title="`${$page.props.auth?.user?.name}`">
        ...
        <ul class="list-inside">
          ...
          <template v-for="duty in user.duties">
            <li v-for="role in duty.roles" :key="role.id">
              <strong>{{ $t(role.name) }}</strong> ({{
                `iš pareigybės „${duty.name}“, kuri yra iš ${duty.institution?.padalinys?.shortname ?? "nežinomo padalinio"
                }`
              }})
    ...
    <p class="mt-4 flex items-center justify-center gap-2">
      <a target="_blank" href="https://github.com/vu-sa/vusa.lt/blob/main/CHANGELOG.md">
        <NButton text size="small" quaternary> v0.4.14 </NButton>
      </a>·
      <a class="inline-flex items-center" target="_blank" href="https://github.com/vu-sa/vusa.lt/pull/122">
