<template>
  <section class="space-y-6" role="region" aria-labelledby="user-timeline-heading">
          ...
          <DropdownMenuContent align="end" class="w-64">
            ...
            <div class="px-2 py-1 max-h-64 overflow-y-auto space-y-1">
              <DropdownMenuCheckboxItem 
                v-for="t in availableTenantsUser" 
                :key="t.id"
                :model-value="tenantFilter.includes(String(t.id))"
                @update:model-value="(val: boolean) => toggleTenantFilter(String(t.id), val)"
                @select.prevent
              >
                {{ t.shortname ?? t.id }}
              </DropdownMenuCheckboxItem>
