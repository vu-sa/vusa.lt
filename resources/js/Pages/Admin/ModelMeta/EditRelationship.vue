<template>
  <RelationshipForm
    :relationship
    enable-delete
    @submit:form="(form) => (form as InertiaForm<App.Entities.Relationship>).patch(route('relationships.update', relationship.id), { preserveScroll: true })"
        ...
        <div v-if="!isEditing" class="space-y-2">
          ...
          <Popover v-model:open="sourceOpen">
            <PopoverTrigger as-child>
              <Button type="button" variant="outline" role="combobox" class="w-full justify-between font-normal pointer-coarse:min-h-11">
...
const accessExplanation = computed(() => {
  ...
  baseExplanation += `<br/><span class="font-medium">${relationForm.bidirectional
