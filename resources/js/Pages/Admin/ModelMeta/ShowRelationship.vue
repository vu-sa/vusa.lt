<template>
  <RecordPage v-model:section="section" :title="relationship.name" :entity-type="ModelEnum.RELATIONSHIP" :facts="facts" :sections="sections" :primary-action="primaryAction" :overflow-actions="overflowActions" @action="handleAction">
    <template #overview>
      <div class="max-w-3xl space-y-5">
        <div>
          <h2 class="mb-2 text-base font-semibold">{{ $t('Aprašymas') }}</h2>
          <p class="whitespace-pre-wrap text-sm">{{ relationship.description || $t('Aprašymo nėra.') }}</p>
        </div>
        <div class="border border-border p-4"><span class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{{ $t('Techninė žymė') }}</span><p class="mt-1 text-sm font-medium">{{ relationship.slug }}</p></div>
      </div>
    </template>
    <template #connections>
      <div class="max-w-4xl divide-y divide-border border-y border-border">
        <div v-for="connection in relationship.relationshipables" :key="connection.id" class="grid gap-2 py-4 sm:grid-cols-[1fr_auto_1fr] sm:items-center">
          <p class="text-sm font-medium">{{ label(connection.source) }}</p>
          <p class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{{ connection.bidirectional ? '↔' : '→' }} {{ connection.scope ?? $t('Bendras') }}</p>
          <p class="text-sm font-medium sm:text-right">{{ label(connection.target) }}</p>
        </div>
        <p v-if="!relationship.relationshipables.length" class="py-6 text-sm text-muted-foreground">{{ $t('Ryšių tarp įrašų nėra.') }}</p>
      </div>
    </template>
  </RecordPage>
