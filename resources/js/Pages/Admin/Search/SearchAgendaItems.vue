<template>
  <AdminContentPage :breadcrumb-options>
    <AdminSearchLayout :title="$t('Darbotvarkės punktų paieška')"
      :description="$t('Ieškokite darbotvarkės punktų pagal temą, posėdį ar sprendimą')"
      :query="searchController.query.value" :search-placeholder="$t('Ieškoti darbotvarkės punktų...')"
      ...
      <template #results>
        <AdminSearchResults :is-searching="searchController.isSearching.value"
          :has-results="searchController.hasResults.value" :has-searched
          :has-active-filters="searchController.hasActiveFilters.value" :error="searchController.error.value"
          :empty-message="$t('Nerasta darbotvarkės punktų pagal jūsų paiešką')" layout="grid" :grid-cols="2"
          @retry="searchController.refresh" @clear-filters="searchController.clearFilters">
          <AgendaItemSearchCard v-for="item in typedResults" :key="item.id" :item />
        </AdminSearchResults>
      </template>
    </AdminSearchLayout>
  </AdminContentPage>
...
import AgendaItemSearchCard from '@/Features/Admin/AdminSearch/Components/Cards/AgendaItemSearchCard.vue';
import type { AgendaItemSearchResult } from '@/Composables/useAdminSearch';
import { AgendaItemIcon } from '@/Components/icons';

// Breadcrumbs
const breadcrumbOptions = [
  { label: $t('Paieška'), icon: AgendaItemIcon },
...
// Initialize search controller for agenda_items collection
...
// Type the results as AgendaItemSearchResult[]
const typedResults = computed(() => {
  return searchController.results.value as AgendaItemSearchResult[];
});
