<template>
  <AdminContentPage>
    ...
    <DutyHero 
      :duty="duty"
      :current-members="filteredUsers.currentUsers"
      :historical-members="filteredUsers.oldUsers"
      :can-assign-members="canAssignMembers"
      :can-manage-duty="canManageDuty"
      @assign-member="showAssignMemberModal = true"
      @manage-duty="handleEdit"
    >
      <template #actions>
        <MoreOptionsButton
          edit
    ...
    <Tabs v-model="currentTab" class="space-y-6">
      <TabsList class="gap-2">
        ...
        <TabsTrigger value="files" v-if="duty.types && duty.types.length > 0">
          {{ $t('Failai') }}
        </TabsTrigger>
      </TabsList>
      ...
      <TabsContent value="overview" class="space-y-6">
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div class="xl:col-span-2 space-y-6">
            <!-- Current Members -->
            <MemberGrid
              :title="$t('Dabartiniai pareigų ėjėjai')"
              :subtitle="`${filteredUsers.currentUsers.length} / ${duty.places_to_occupy || 0} ${$t('pozicijų')}`"
              :members="filteredUsers.currentUsers"
              :max-positions="duty.places_to_occupy"
              :show-contact="true"
              :show-actions="true"
              :can-edit="canManageDuty"
              :can-add-member="canAssignMembers"
              :empty-title="$t('Pareigos neužimtos')"
              :empty-description="$t('Šiose pareigose šiuo metu niekas nedarbuojasi.')"
              @add-member="showAssignMemberModal = true"
            ...
            <Card v-if="emptyPositions > 0">
              ...
              <CardContent>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div
                    v-for="(empty, index) in emptyPositions"
                    :key="index"
            ...
            <Card>
              ...
              <CardContent class="space-y-4">
                <div class="flex items-center justify-between">
                  <span class="text-sm text-zinc-600 dark:text-zinc-400">{{ $t('Užimtumas') }}</span>
                  <span class="font-medium">{{ `${filteredUsers.currentUsers.length}/${duty.places_to_occupy || 0}` }}</span>
                </div>
                <div class="flex items-center justify-between">
                  <span class="text-sm text-zinc-600 dark:text-zinc-400">{{ $t('Laisvų pozicijų') }}</span>
                  <span class="font-medium">{{ emptyPositions }}</span>
                </div>
                <div class="flex items-center justify-between">
                  <span class="text-sm text-zinc-600 dark:text-zinc-400">{{ $t('Viso narių istoriją') }}</span>
                  <span class="font-medium">{{ allMembers.length }}</span>
                </div>
                <div class="flex items-center justify-between">
                  <span class="text-sm text-zinc-600 dark:text-zinc-400">{{ $t('Kontaktinis el. paštas') }}</span>
                  <span class="font-medium">{{ duty.email ? '✓' : '✗' }}</span>
                </div>
              </CardContent>
            </Card>

            <!-- Contact Info -->
            <Card v-if="duty.email">
              <CardHeader>
      ...
      <TabsContent value="members">
        <MemberGrid
          :title="$t('Visi dabartiniai pareigų ėjėjai')"
      ...
      <TabsContent value="files" v-if="duty.types && duty.types.length > 0">
        <Suspense>
