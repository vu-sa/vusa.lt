<template>
  ...
  <Form class="flex flex-col gap-6" @submit="onSubmit" v-slot="{ errors }" :initial-values="initialValues.value">
      <!-- Quick Institution Selection as mini cards/badges -->
      <div class="space-y-4">
        <h3 class="text-sm font-medium text-muted-foreground">{{ $t('Jūsų institucijos') }}</h3>
        <div class="flex flex-wrap gap-2">
          <Button
            v-for="inst in myInstitutions"
            :key="`mine-${inst.value}`"
            type="button"
            size="sm"
            variant="secondary"
          ...
          >
            ...
            <span class="truncate max-w-[14rem]">{{ inst.label }}</span>
          </Button>
        </div>
      </div>
      ...
      <div v-if="hasAdditionalInstitutions" class="space-y-2">
        <h3 class="text-sm font-medium text-muted-foreground">{{ $t('Kitos institucijos') }}</h3>
                ...
                <template v-for="institution in filteredInstitutions" :key="institution?.value">
                  ...
                  >
                    <div class="flex items-start gap-2 w-full">
                      <component :is="Icons.INSTITUTION" class="h-4 w-4 mt-0.5 flex-shrink-0" />
                      <div class="flex-1 min-w-0">
                        <p class="font-medium">{{ institution.label }}</p>
                        <p v-if="institution.context" class="text-xs text-muted-foreground">
                          {{ institution.context }}
                        </p>
                      </div>
                      <Badge v-if="institution.origin === 'admin'" variant="outline" class="shrink-0 text-[10px]">Admin</Badge>
                    </div>
                  </Button>
                </template>
              </div>
          ...
          <FormItem>
            ...
            <div v-if="selectedInstitution && selectedInstitutionData" class="mt-3 p-3 bg-muted/50 rounded-lg">
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">
                  <h4 class="font-medium">{{ selectedInstitutionData.name }}</h4>
                  <div class="mt-2 space-y-1 text-sm text-muted-foreground">
                    <div v-if="selectedInstitutionData.type" class="flex items-center gap-2">
                      <Badge variant="secondary" class="text-xs">{{ selectedInstitutionData.type }}</Badge>
                    </div>
