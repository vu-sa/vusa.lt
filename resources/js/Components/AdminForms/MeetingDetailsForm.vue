<template>
  <div class="space-y-6">
    <!-- Time Suggestions based on historical patterns -->
    <div v-if="timeSuggestions.length > 0 && !selectedMeeting.start_time" class="space-y-2">
      <p class="text-xs text-muted-foreground">
        {{ $t('Rekomenduojami laikai pagal ankstesnius susitikimus') }}
      </p>
      <div class="grid grid-cols-1 gap-2">
        <Button v-for="suggestion in timeSuggestions.slice(0, 4)" :key="suggestion.value" type="button"
          variant="outline" size="sm" class="justify-start text-left h-auto p-3"
          @click="applyTimeSuggestion(suggestion)">
          <div class="flex items-center gap-3 w-full">
            ...
            <div class="flex-1">
              ...
              <p v-if="suggestion.is_pattern" class="text-xs text-green-600 dark:text-green-400">
                <span class="inline-flex items-center gap-1">
                  <component :is="Icons.CHECK_CIRCLE" class="h-3 w-3" />
                  {{ $t('Pagal šablonį') }} ({{ suggestion.frequency }}x)
                </span>
              </p>
              <p v-else class="text-xs text-muted-foreground">
                {{ $t('Standartinis laikas') }}
              </p>
            </div>
          </div>
        </Button>
    ...
    <Form v-slot="{ values, setFieldValue }" :validation-schema="schema" :initial-values @submit="onSubmit">
...
const timeSuggestions = ref<any[]>([]);

// Computed properties
const selectedMeeting = computed(() => props.meeting || {});
...
const schema = toTypedSchema(
  z.object({
    ...
    .refine((data) => {
      // Business hours validation (optional - can be removed if not needed)
      if (data.time) {
      ...
      message: $t("Rekomenduojamas laikas: 7:00 - 22:00"),
      path: ["time"]
    })
);
...
// Methods
const applyTimeSuggestion = (suggestion: any) => {
  const suggestedDateTime = new Date(suggestion.datetime);

  // Time suggestion applied - no need to update form state as vee-validate handles it
};

const showAlternativeTimes = async () => {
  if (props.institutionId) {
    try {
      const url = new URL('/api/v1/meetings/suggestions', window.location.origin)
      url.searchParams.set('institution', String(props.institutionId))
      const { data, error } = await useFetch(url.toString(), {
        immediate: true,
      }).get().json()
      if (!error.value) timeSuggestions.value = data.value ?? []
    } catch (error) {
      console.warn('Failed to load alternative times:', error);
    }
  }
};
...
const formatSuggestionDate = (dateString: string): string => {
...
const formatSuggestionTime = (dateString: string): string => {
  const date = new Date(dateString);
...
const formatSelectedDateTime = (date: Date, time: TimeValue): string => {
...
// Lifecycle
onMounted(() => {
  // Meeting types are now provided via props, no need to fetch
});
</script>
