...
export function setupDefs(ctx: DefsContext): void {
  ...
  // Diagonal stripe pattern for check-ins (amber theme, subtle)
  const checkInPattern = defs.append('pattern')
    .attr('id', 'checkInStripes')
    .attr('patternUnits', 'userSpaceOnUse')
    .attr('width', 6)
    .attr('height', 6)
    .attr('patternTransform', 'rotate(-45)')
  checkInPattern.append('rect')
    .attr('width', 6)
    .attr('height', 6)
    .attr('fill', 'transparent')
  checkInPattern.append('line')
    .attr('x1', 0)
    .attr('y1', 0)
    .attr('x2', 0)
    .attr('y2', 6)
    .attr('stroke', colors.checkInFill)
    .attr('stroke-width', 3)
}
