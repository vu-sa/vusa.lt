...
type LayoutRow = { key: string | number; type: Row['type']; tenantId?: string|number; institutionId?: string|number; top: number; height: number }
...
// Get vacation periods for the timeline
const getVacationPeriods = (minDate: Date, maxDate: Date) => {
  const periods: { start: Date; end: Date; type: string }[] = []
  ...
  for (let year = startYear; year <= endYear; year++) {
    ...
    // Late January vacation: last week of January until February 4
...
const render = () => {
  ...
  // Vacation period bands (subtle background for vacation times)
  ...
  // gaps as stroked lines (to match legend style)
  ...
  .attr('y2', d => rowCenter(d.institution_id))
    ...
    .attr('stroke', (d: any) => {
      // If confidence is provided (0..1), interpolate from amber-200 to red-500
      if (typeof d.confidence === 'number') {
        const c = Math.max(0, Math.min(1, d.confidence))
        return d3.interpolateRgb('#fde68a', '#ef4444')(c)
      }
      return (d.mode === 'heads_up' ? 'rgba(245, 158, 11, 0.9)' : 'rgba(239, 68, 68, 0.9)')
    })
    ...
    .append('title')
    .text(d => `${d.mode ?? 'gap'}: ${d.fromDate.toLocaleDateString()} → ${d.untilDate.toLocaleDateString()}`)
  ...
  dots.append('title').text(d => d.title || labelFor(d.institution_id) || new Date(d.date).toLocaleString())
  ...
  dots
    ...
    .on('mouseleave', function () { tipDiv.classed('hidden', true) })
...
// Handle near-edge scrolling to extend range
function onScroll() {
...
// Handle zoom/scale via slider; preserve center anchor date
...
<template>
  <div ref="wrap" class="relative w-full max-w-full">
    ...
    <div class="flex items-center justify-between gap-3 mb-2">
      <div class="flex items-center gap-4 min-w-0">
        <div v-if="showLegend" class="flex flex-wrap items-center gap-4 text-[11px] text-slate-600">
          <div class="flex items-center gap-1"><span class="inline-block w-2 h-2 rounded-full bg-black"></span><span>{{ $t('meeting') || 'Meeting' }}</span></div>
          <div class="flex items-center gap-1"><span class="inline-block w-4 h-[2px] rounded-full bg-amber-500"></span><span>{{ $t('anticipated_gap') }}</span></div>
          <div class="flex items-center gap-1" :title="$t('Summer vacation') + ', ' + $t('Winter vacation') + ', ' + $t('Easter vacation')">
            <span class="inline-block w-4 h-[2px] rounded-sm bg-gradient-to-r from-amber-400/30 via-blue-300/30 to-violet-300/30"></span>
        ...
        <div class="flex items-center gap-2 text-[11px] text-slate-600 truncate">
          <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded border border-slate-200 bg-white/70">{{ $t('Institucijų') }}: {{ layoutRows.filter(r => r.type === 'institution').length }}</span>
          <template v-if="tenantFilter?.length">
            <span class="opacity-70">•</span>
            <div class="flex items-center gap-1 truncate">
              <span class="opacity-70 shrink-0">{{ $t('Filtras') }}:</span>
              <div class="flex items-center gap-1 overflow-hidden">
                <span v-for="tid in tenantFilter" :key="String(tid)" class="px-1.5 py-0.5 rounded bg-slate-100 text-slate-700 border border-slate-200 truncate">
                  {{ tenantNames?.[tid] ?? tid }}
                </span>
              </div>
            </div>
      ...
      <div class="flex items-center gap-3 ml-auto">
        ...
        <label class="flex items-center gap-1 text-[11px] text-slate-600 select-none cursor-pointer">
          <input type="checkbox" :checked="!!detailsExpanded" @change="emit('update:detailsExpanded', !detailsExpanded)" />
        ...
        <div class="w-40 flex items-center gap-2 text-[11px] text-slate-600">
          ...
          <Slider :min="6" :max="72" :step="2" :model-value="[dayWidthPx || dayWidth]" @update:model-value="onScaleChange" />
        </div>
        <!-- Sticky current year badge -->
        <div v-if="currentYear" class="px-2 py-0.5 rounded border text-xs text-slate-700 bg-white/70 backdrop-blur border-slate-200">
          {{ currentYear }}
        </div>
        <button type="button" class="px-2 py-1 text-xs border rounded hover:bg-slate-50 dark:hover:bg-slate-900" @click="$emit('fullscreen', true)">
          {{ $t('Visas ekranas') }}
        </button>
      </div>
    </div>
    ...
    <div class="flex w-full max-w-full border rounded-md overflow-hidden" 
         ...
         :class="{ 'h-full': props.height === '100%' }" 
         style="min-width: 0;">
      <!-- Left: sticky labels -->
      <div ref="leftLabels" class="shrink-0 bg-white sticky left-0 z-[1] overflow-hidden" :style="{ width: `${labelWidth}px` }">
        <div class="grid" :style="{ gridTemplateRows: `22px ${layoutRows.map(r => r.height + 'px').join(' ')}` }">
          <!-- header spacer (align with axis height) -->
          <div class="border-b border-slate-200 bg-white sticky top-0 z-20"></div>
          <template v-for="(row, idx) in layoutRows" :key="`label-${row.key}`">
            <div v-if="row.type === 'tenant'" class="px-3 py-1 text-xs font-medium text-slate-600 border-b border-slate-200 bg-slate-50/70 sticky top-[22px] z-[2]">
              {{ tenantNames?.[row.tenantId!] ?? row.tenantId }}
            </div>
            <div v-else class="px-3 py-1 text-sm text-slate-700 border-b border-slate-100 flex items-start gap-2 truncate"
                 :class="[idx % 2 === 0 ? 'bg-slate-50/40' : '']" :title="labelFor(row.institutionId!)">
              <div class="flex-1 min-w-0">
                <div class="flex items-center justify-between gap-2">
                  <button type="button" class="truncate text-left hover:underline cursor-pointer focus:underline focus:outline-none"
                  ...
                  <span class="text-[11px] text-slate-500 shrink-0" v-if="lastMeetingByInstitution.get(row.institutionId!)">{{ labelLast(lastMeetingByInstitution.get(row.institutionId!)!) }}</span>
                </div>
...
<style scoped>
svg :global(text) { font-size: 10px; fill: rgb(113,113,122); }
