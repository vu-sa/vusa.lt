...
import { ArrowUpRight, Boxes, Landmark, UserPlus, Users } from 'lucide-vue-next';
import { computed } from 'vue';

import type { HomeRegistrationForm } from './types';

import { NavigationTiles, type NavigationTileItem } from '@/Components/Patterns';
import { useAdminNavigation, workspaceHref } from '@/Composables/useAdminNavigation';
...
const links = computed<NavigationTileItem[]>(() => {
  const visak = workspaces.value.find(workspace => workspace.key === 'atstovavimas');
  const visakHref = visak ? workspaceHref(visak) : undefined;

  return [
    visak && visakHref
      ? { key: 'visak', label: visak.label, description: visak.description, href: visakHref, icon: Landmark }
      : null,
