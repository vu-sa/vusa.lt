  ...
  <Dialog :open="open" @update:open="$emit('close')">
    <DialogContent class="sm:max-w-lg">
      <DialogHeader>
        <div class="flex items-center gap-3">
          ...
          <div class="min-w-0 flex-1">
            <DialogTitle class="text-base">{{ task.name }}</DialogTitle>
      ...
      <div v-if="task.description" class="rounded-lg border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-700 dark:bg-zinc-800/50">
        <div class="flex gap-3">
          ...
          <div class="text-sm text-zinc-700 dark:text-zinc-300">
            <p class="font-medium text-zinc-900 dark:text-zinc-100">{{ $t('Ką reikia padaryti?') }}</p>
      ...
      <div v-if="isPeriodicityGapTask && !task.completed_at" class="space-y-3">
        <p class="text-sm font-medium text-zinc-700 dark:text-zinc-300">{{ $t('Pasirinkite veiksmą:') }}</p>
        <div class="grid gap-2 sm:grid-cols-2">
          ...
          >
            ...
            <div>
              <p class="font-medium text-emerald-700 dark:text-emerald-300">{{ $t('tasks.periodicity_gap.action_schedule_meeting') }}</p>
              <p class="mt-0.5 text-xs text-emerald-600/80 dark:text-emerald-400/80">{{ $t('Sukurti naują posėdį šiai institucijai') }}</p>
            </div>
          </Button>
          <Button 
            variant="outline" 
          ...
          >
            ...
            <div>
              <p class="font-medium text-amber-700 dark:text-amber-300">{{ $t('tasks.periodicity_gap.action_report_no_meeting') }}</p>
              <p class="mt-0.5 text-xs text-amber-600/80 dark:text-amber-400/80">{{ $t('Pranešti, kad posėdžių nebus tam tikrą laikotarpį') }}</p>
            </div>
          </Button>
        </div>
      </div>
      ...
      <div v-if="task.users?.length" class="space-y-2">
        <p class="text-sm font-medium text-zinc-700 dark:text-zinc-300">{{ $t('forms.fields.responsible_people') }}</p>
