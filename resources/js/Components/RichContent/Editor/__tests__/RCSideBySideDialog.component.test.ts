...
describe('RCSideBySideDialog', () => {
  ...
  it('hides the width picker entirely for blocks with a single allowed width (e.g. hero-carousel)', () => {
    const wrapper = mount(RCSideBySideDialog, {
      props: {
        open: true,
        content: { id: 2, type: 'hero-carousel', json_content: [], options: { width: 'full' } },
      },
      global: {
        stubs: { ...commonStubs, ContentEditorFactory: ContentEditorFactoryStub, BlockPreviewRenderer: BlockPreviewRendererStub },
      },
    ...
    expect(anyWidthOption, 'hero-carousel only allows "full", so the picker should not render').toBeUndefined();
  });
