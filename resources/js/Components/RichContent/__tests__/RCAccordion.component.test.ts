...
describe('RCAccordion — editable (full-screen editor)', () => {
  // RCInlineText/RCAddPlaceholder are lazy-loaded (see RCAccordion.vue) — resolving that
  // dynamic import needs a wait before the editable markup they render exists.
  async function mountEditable(element: ShadcnAccordion, activeInlineField: string | null = null) {
    const wrapper = mount(RCAccordion, {
  ...
  it('starts every item collapsed, same as the public accordion', async () => {
    const wrapper = await mountEditable(makeElement([{ label: 'A' }, { label: 'B' }]));
  ...
  it('renders the trigger as a plain div, not a nested button, and the label inline-editable', async () => {
    const wrapper = await mountEditable(makeElement([{ label: 'Klausimas' }]));
  ...
  it('emits update:element with the patched label, preserving content', async () => {
    const wrapper = await mountEditable(makeElement([{ label: 'Old' }]));
    const label = wrapper.find('[contenteditable]');
  ...
  it('shows a placeholder for an opened item with no content yet', async () => {
    const wrapper = await mountEditable(makeElement([{ label: 'A' }]));
  ...
  it('mounts a live TiptapEditor for the claimed, open item, and bubbles its edits', async () => {
    const wrapper = await mountEditable(makeElement([{ label: 'A' }, { label: 'B' }]), 'acc-1:content:1');
  ...
  it('places the remove button beside each item, not inside its card', async () => {
    const wrapper = await mountEditable(makeElement([{ label: 'A' }]));
