<template>
  <FilterSidebar :active-filter-count mobile-title="search.filter_meetings" @clear-filters="emit('clearFilters')">
    <template #mobile-filters>
      <Accordion type="multiple" :default-value="['tenants', 'years', 'dates']" class="w-full">
        <!-- Tenant Filter -->
        <FilterAccordion value="tenants" :label="$t('search.tenants')" :icon="Building2"
          :badge-count="filters.tenants.length" variant="mobile">
        ...
        <FilterAccordion v-if="institutionTypeFacet?.values?.length" value="institution-types"
          :label="$t('search.institution_type')" :icon="Layers" :badge-count="filters.institutionTypes.length"
          variant="mobile">
          <CheckboxFilter :options="institutionTypeOptions" :selected-values="filters.institutionTypes" :max-visible="0"
            @toggle="(value) => emit('update:institutionType', String(value))" />
        </FilterAccordion>

        <!-- Years Filter (Mobile) -->
        <FilterAccordion value="years" :label="$t('search.years')" :icon="CalendarDays"
          :badge-count="filters.years.length" variant="mobile">
          <YearFilter :values="yearFacet?.values || []" :selected-values="filters.years" :max-visible="0"
            @toggle="(year) => emit('update:year', year)" />
        </FilterAccordion>

        <!-- Date Range Filter (Mobile) -->
        <FilterAccordion value="dates" :label="$t('search.date')" :icon="Calendar" :badge-count="hasDateFilter ? 1 : 0"
          variant="mobile">
    ...
    <template #desktop-filters>
      <Accordion type="multiple" :default-value="['tenants', 'years', 'dates']" class="w-full space-y-3">
        <!-- Tenant Filter -->
        <FilterAccordion value="tenants" :label="$t('search.tenants')" :icon="Building2"
          :badge-count="filters.tenants.length" :is-loading :icon-container-class="getFacetIconColor('Building2')">
          <TenantFilter :tenant-hierarchy="processedTenantHierarchy" :selected-tenants="filters.tenants"
            @toggle-tenant="(tenant: string) => emit('update:tenant', tenant)" />
        </FilterAccordion>
        ...
        <FilterAccordion v-if="institutionTypeFacet?.values?.length" value="institution-types"
          :label="$t('search.institution_type')" :icon="Layers" :badge-count="filters.institutionTypes.length"
          :is-loading :icon-container-class="getFacetIconColor('Layers')">
          <CheckboxFilter :options="institutionTypeOptions" :selected-values="filters.institutionTypes" :max-visible="0"
            @toggle="(value) => emit('update:institutionType', String(value))" />
        </FilterAccordion>
        ...
        <!-- Years Filter -->
        <FilterAccordion value="years" :label="$t('search.years')" :icon="CalendarDays"
          :badge-count="filters.years.length" :is-loading :icon-container-class="getFacetIconColor('CalendarDays')">
          <YearFilter :values="yearFacet?.values || []" :selected-values="filters.years" :max-visible="8"
            @toggle="(year) => emit('update:year', year)" />
        </FilterAccordion>
...
import { FilterSidebar, FilterAccordion, CheckboxFilter, YearFilter, getFacetIconColor } from '@/Components/Shared/Search';
