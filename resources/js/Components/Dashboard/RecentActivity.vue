<template>
  <div class="space-y-4">
    ...
    <div v-else-if="error" class="flex flex-col items-center justify-center py-8">
      ...
      <p class="mt-2 text-center text-muted-foreground">{{ $t('Nepavyko užkrauti veiklos') }}</p>
      <Button @click="fetchActivities" variant="outline" class="mt-4">
        {{ $t('Bandyti dar kartą') }}
      </Button>
    </div>
    ...
    <div v-if="hasMoreActivities" class="flex justify-center pt-2">
      <Button @click="loadMore" variant="ghost" size="sm" :disabled="loadingMore">
