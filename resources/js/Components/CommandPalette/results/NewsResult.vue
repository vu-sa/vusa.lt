<template>
  <CommandItem
    :value="itemValue"
    class="group cursor-pointer rounded-lg px-3 py-2.5 transition-colors hover:bg-accent data-[highlighted]:bg-accent"
    @select="handleSelect"
  >
    <div class="flex items-center gap-3 w-full min-w-0">
      <!-- Icon container with background -->
      <div class="flex size-9 shrink-0 items-center justify-center rounded-lg bg-amber-500/10 text-amber-600 dark:bg-amber-500/20 dark:text-amber-400 group-hover:bg-amber-500/15 dark:group-hover:bg-amber-500/25 transition-colors">
        <Newspaper class="size-4" />
      </div>

      <!-- Content -->
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2">
          <span class="font-medium truncate text-sm group-hover:text-foreground transition-colors">
            {{ news.title || $t('Be pavadinimo') }}
          </span>
          <span
            v-if="news.lang"
            class="inline-flex items-center rounded-full px-1.5 py-0.5 text-[10px] font-medium ring-1 ring-inset bg-zinc-500/10 text-zinc-600 ring-zinc-500/20 dark:bg-zinc-500/20 dark:text-zinc-400 uppercase"
          >
            {{ news.lang }}
          </span>
        </div>
        <div class="flex items-center gap-1.5 text-xs text-muted-foreground mt-0.5">
          <span v-if="news.tenant_name" class="shrink-0 font-medium">{{ news.tenant_name }}</span>
          <span v-if="news.tenant_name && formattedDate" class="text-muted-foreground/40">•</span>
          <span v-if="formattedDate" class="shrink-0 tabular-nums">{{ formattedDate }}</span>
        </div>
      </div>

      <!-- Arrow indicator -->
      <ChevronRight class="size-4 text-muted-foreground/40 opacity-0 group-hover:opacity-100 transition-opacity shrink-0" />
    </div>
  </CommandItem>
</template>

<script setup lang="ts">
import { computed } from 'vue'
import { router } from '@inertiajs/vue3'
import { trans as $t } from 'laravel-vue-i18n'
import { ChevronRight, Newspaper } from 'lucide-vue-next'
import { CommandItem } from '@/Components/ui/command'
import { useCommandPalette, type RecentItem } from '@/Composables/useCommandPalette'
import type { NewsSearchResult } from '@/Composables/useAdminSearch'

const props = defineProps<{
  news: NewsSearchResult
}>()

const { close, addRecentItem } = useCommandPalette()

const itemValue = computed(() => `news-${props.news.id}`)

const formattedDate = computed(() => {
  if (!props.news.publish_time) return null
  const date = new Date(props.news.publish_time * 1000)
  return date.toLocaleDateString('lt-LT', {
    year: 'numeric',
    month: 'short',
    day: 'numeric'
  })
})

const handleSelect = () => {
  // Add to recent items
  addRecentItem({
    id: props.news.id,
    type: 'news',
    title: props.news.title || $t('Be pavadinimo'),
    href: route('news.edit', props.news.id)
  } as Omit<RecentItem, 'timestamp'>)

  // Navigate to edit page
  close()
  router.visit(route('news.edit', props.news.id))
}
</script>
