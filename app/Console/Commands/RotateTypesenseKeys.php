...
class RotateTypesenseKeys extends Command
{
    ...
    {
        $this->info('🔄 Typesense Key Rotation');
        $this->newLine();
        ...
        if (! $this->option('force')) {
            $this->newLine();
            $this->warn('⚠️  This will generate new API keys and update your .env file.');
        ...
        try {
            ...
            $this->newLine();
            $this->info('📦 Step 1/4: Generating new public search-only key...');
            ...
            $this->newLine();
            $this->info('📦 Step 2/4: Generating new admin search-only key...');
            ...
            $this->newLine();
            $this->info('📦 Step 3/4: Invalidating cached scoped keys...');
            $this->invalidateScopedKeyCache();

            if ($this->option('delete-old') && ! empty($oldKeyIds)) {
                $this->newLine();
                $this->info('📦 Step 4/4: Deleting old keys from Typesense...');
                $this->deleteOldKeys($client, $oldKeyIds);
            } else {
                $this->info('📦 Step 4/4: Skipping old key deletion (use --delete-old to enable)');
            }
    ...
    private function showCurrentKeyInfo(): void
    {
        ...
        $this->table(
            ['Key Type', 'Status'],
            [
                ['Public Search Key', $publicKey ? '✅ Configured' : '❌ Not set'],
                ['Admin Search Key', $adminSearchKey ? '✅ Configured' : '❌ Not set'],
            ]
        );
    }
    ...
    private function generatePublicKey(Client $client): int
    {
        ...
        $this->info('   ✅ Public key generated: '.substr($keyResponse['value'], 0, 8).'...');
    ...
    private function generateAdminKey(Client $client): int
    {
        ...
        $this->info('   ✅ Admin key generated: '.substr($keyResponse['value'], 0, 8).'...');
        ...
        if ($cacheDriver === 'redis') {
            ...
            try {
                ...
                if (! empty($keys)) {
                    ...
                    $redis->del($keys);
                    $this->info('   ✅ Cleared '.count($keys).' cached scoped keys');

                    return;
                }

                $this->info('   ℹ️  No cached scoped keys found');

                return;
            } catch (\Exception $e) {
                // Fall through to manual note
                $this->warn('   ⚠️  Could not clear Redis cache: '.$e->getMessage());
            }
        }

        $this->info('   ℹ️  Scoped keys will regenerate on next user request (cache TTL ~1 hour)');
        $this->info('   💡 Run "sail artisan cache:clear" to force immediate regeneration for all users');
    }
    ...
    private function deleteOldKeys(Client $client, array $keyIds): void
    {
        ...
        $this->info("   ✅ Deleted {$deleted} old key(s) from Typesense");
    }
    ...
    {
        $this->info('📚 Collection Configuration:');
