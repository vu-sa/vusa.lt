<?php

return [
    // Search interface
    'search' => 'Paieška',
    'search_placeholder' => 'Ieškokite naujienų, puslapių, dokumentų, renginių...',
    'search_placeholder_alt' => 'Ieškokite naujienų, dokumentų, renginių...',
    'search_across_types' => 'Ieškokite naujienose, puslapiuose, dokumentuose ir renginiuose. Naudokite rodyklių klavišus rezultatų navigacijai.',
    'search_across_selected' => 'Ieškokite pasirinktuose turinio tipuose',
    'searching' => 'Ieškoma...',
    'search_unavailable' => 'Paieška šiuo metu nepasiekiama. Bandykite dar kartą vėliau.',
    'start_search' => 'Pradėkite paiešką',

    // Search actions
    'clear_search' => 'Išvalyti paiešką',
    'click_to_open' => 'Spustelėkite atidaryti',
    'open_close_search' => 'Atidaryti/Uždaryti paiešką',
    'toggle_filters' => 'Perjungti filtrus',
    'toggle_view_mode' => 'Perjungti peržiūros režimą',
    'show_keyboard_shortcuts' => 'Rodyti klaviatūros sparčiuosius klavišus',

    // Content type toggles
    'toggle_news' => 'Perjungti naujienas',
    'toggle_pages' => 'Perjungti puslapius',
    'toggle_documents' => 'Perjungti dokumentus',
    'toggle_events' => 'Perjungti renginius',
    'toggle_institutions' => 'Perjungti institucijas',

    // Keyboard shortcuts
    'keyboard_shortcuts' => 'Klaviatūros spartieji klavišai',

    // Results and stats
    'results' => 'rezultatai',
    'items_found' => 'elementai rasti',
    'total_results' => 'Viso rezultatų',
    'all_results' => 'Visi rezultatai',
    'no_results_found' => 'Jūsų paieškai rezultatų nerasta',
    'no_results_found_for_your_search' => 'Jūsų paieškai rezultatų nerasta',
    'load_more_results' => 'Įkelti daugiau rezultatų',
    'loading' => 'Kraunama',
    'show_more_type' => 'Rodyti daugiau :type',
    'untitled' => 'Be pavadinimo',
    'keep_typing_to_see_results' => 'Tęskite rašymą rezultatams matyti',
    'please_enter_at_least_3_characters' => 'Įveskite bent 3 simbolius',
    'search_across_your_selected_content_types' => 'Ieškokite pasirinktuose turinio tipuose',

    // Filters and sorting
    'search_in' => 'Ieškoti',
    'select_all' => 'Pažymėti viską',
    'deselect_all' => 'Atžymėti viską',
    'sort_results_by' => 'Rūšiuoti rezultatus pagal',
    'sort_results' => 'Rūšiuoti rezultatus',
    'sort_by' => 'Rūšiuoti pagal...',
    'display' => 'Rodymas',
    'group_results_by_type' => 'Grupuoti rezultatus pagal turinio tipą',
    'quick_filters' => 'Spartieji filtrai',
    'quick_actions' => 'Spartieji veiksmai',
    'active_filters' => 'Aktyvūs filtrai',
    'reset_filters' => 'Atstatyti filtrus',
    'clear_all' => 'Išvalyti viską',
    'select_all' => 'Pasirinkti viską',
    'deselect_all' => 'Nuimti pažymėjimą nuo visų',

    // Sort options
    'relevance' => 'Relevance',
    'date_newest' => 'Data (naujausios)',
    'date_oldest' => 'Data (seniausi)',
    'title_az' => 'Pavadinimas (A-Ž)',

    // Filter options
    'language' => 'Kalba',
    'institution' => 'Institucija',
    'date_range' => 'Datos intervalas',
    'has_images' => 'Su nuotraukomis',
    'recent_content' => 'Naujas turinys (per 30 dienų)',
    'searching_in' => 'Ieškoma',

    // Recent searches
    'recent_searches' => 'Paskutinės paieškos',
    'suggestions' => 'Pasiūlymai',
    'clear' => 'Išvalyti',

    // Date range filter
    'select_date_range' => 'Pasirinkti datų intervalą',
    'select_date' => 'Pasirinkti datą',
    'select_year_range' => 'Pasirinkti metų intervalą',
    'clear_range' => 'Išvalyti intervalą',

    // Error messages
    'search_error' => 'Klaida atliekant paiešką',
    'technical_error' => 'Atsiprašome, įvyko techninė klaida. Pabandykite dar kartą.',
    'try_again' => 'Bandyti dar kartą',
    'report_error' => 'Pranešti apie klaidą',

    // Document details
    'effective_date' => 'Įsigalioja:',

    // Navigation
    'next' => 'Kitas',
    'previous' => 'Ankstesnis',
    'show_more' => 'Rodyti daugiau',

    // Voice search
    'start_voice_search' => 'Pradėti balso paiešką',
    'speak_search_query' => 'Ištarkite paieškos tekstą',
    'listening' => 'Klausomasi...',

    // Content type names (in case they need translation)
    'news' => 'Naujienos',
    'pages' => 'Puslapiai',
    'documents' => 'Dokumentai',
    'events' => 'Renginiai',
    'News' => 'Naujienos',
    'Pages' => 'Puslapiai',
    'Documents' => 'Dokumentai',
    'Events' => 'Renginiai',

    // Sort options
    'Relevance' => 'Tinkamumas',
    'Date' => 'Data',
    'Type' => 'Tipas',

    // General actions
    'cancel' => 'Atšaukti',

    // Content type indicators
    'no_content_types_selected' => 'Nepasirinkti jokie turinio tipai',
    'enable_all_types' => 'Įjungti visus',
    'clear_all_types' => 'Išvalyti visus',
    'click_to_disable_type' => 'Spustelėkite išjungti :type',

    // Document search specific
    'document_search_title' => 'Dokumentai',
    'document_search_description' => 'Ieškokite VU SA dokumentų archyve – raskite protokolus, nutarimus ir kitus dokumentus.',
    'document_page_title' => 'Dokumentai - VU SR',
    'document_page_description' => 'Ieškokite VU SA dokumentų archyve',
    'offline_message' => 'Nėra interneto ryšio - paieška nepasiekiama',
    'showing_results' => 'Rodomi',
    'found_results' => 'Surasta rezultatų:',
    'document_singular' => 'dokumentas',
    'document_plural' => 'dokumentai',
    'newest_first' => '(naujausi pirmiausia)',
    'by_query' => 'pagal',
    'no_documents_found' => 'Dokumentų nerasta',
    'min_chars_search' => 'Įveskite bent 3 simbolius paieškai',
    'enter_search_or_browse' => 'Įveskite paieškos žodžius arba naršykite visus dokumentus',
    'view_mode_list' => 'Sąrašas',
    'view_mode_compact' => 'Kompaktiškas',
    'language_lithuanian' => 'LT',
    'language_english' => 'EN',
    'language_unknown' => 'Nenurodyta',

    // Search input specific
    'search_documents_placeholder' => 'Ieškoti dokumentų pagal pavadinimą...',
    'search_documents_placeholder_alt' => 'Ieškoti dokumentų...',
    'search_button' => 'Ieškoti',
    'show_all_button' => 'Rodyti visus',
    'clear_search_button' => 'Išvalyti paiešką',
    'auto_search_enabled' => 'Automatinė paieška įjungta',
    'enable_auto_search' => 'Įjungti automatinę paiešką',
    'disable_auto_search' => 'Išjungti automatinę paiešką',
    'recent_searches' => 'Pastarieji paieškos žodžiai',
    'clear_all_history' => 'Išvalyti viską',

    // Facet sidebar specific
    'filters' => 'Filtrai',
    'filter_documents' => 'Filtruoti dokumentus',
    'clear_filters' => 'Išvalyti',
    'clear_filters_count' => 'Išvalyti (:count)',
    'tenants' => 'Dariniai',
    'document_type' => 'Dokumentų tipas',
    'language' => 'Kalba',
    'date' => 'Data',
    'document_creation_time' => 'Dokumento sukūrimo laikas',
    'language_filters_after_search' => 'Kalbų filtrai bus rodomi atlikus paiešką',

    // Results specific
    'loading_more' => 'Kraunama...',
    'show_more_results' => 'Rodyti daugiau rezultatų',
    'no_results_criteria' => 'Pagal jūsų paieškos kriterijus dokumentų nerasta.',
    'clear_filters_action' => 'Išvalyti filtrus',
    'min_chars_required' => 'Reikalingi bent 3 simboliai',
    'min_chars_description' => 'Įveskite bent 3 simbolius, kad galėtumėte ieškoti dokumentų.',
    'or_browse_all' => 'Arba',
    'browse_all_documents' => 'naršykite visus dokumentus',

    // Filter component specific
    'select_all' => 'Visi',
    'clear_all' => 'Išvalyti',
    'tenants_organizations' => 'Dariniai',
    'units' => 'Padaliniai',
    'other_types' => 'Kiti',
    'most_important' => 'Svarbiausi',
    'date_from' => 'Nuo datos',
    'date_to' => 'Iki datos',
    'from_date' => 'Nuo',
    'to_date' => 'Iki',
    'last_90_days' => 'Paskutiniai 90 dienų',
    'years' => 'Metai',
    'document_type_description' => 'Protokolai, nuostatai, kiti',
    'language_description' => 'Lietuvių, anglų kalba',

    // Date range presets
    'date_recent' => 'Pastarieji',
    'date_3_months' => '3 mėnesiai',
    'date_6_months' => '6 mėnesiai',
    'date_1_year' => 'Metai',
    'date_year_range' => 'Metų intervalas',
    'date_custom' => 'Pasirinkti',
    'date_recent_desc' => '3 mėnesiai',
    'date_3_months_desc' => 'Paskutiniai 90 dienų',
    'date_6_months_desc' => 'Pusmetis atgal',
    'date_1_year_desc' => 'Paskutiniai 12 mėn.',
    'date_year_range_desc' => 'Pasirinkti metų intervalą',
    'date_custom_desc' => 'Konkretus intervalas',
    'date_from_format' => 'Nuo :date',
    'date_to_format' => 'Iki :date',

    // Meeting search specific
    'meeting_page_title' => 'Posėdžiai - VU SR',
    'meeting_page_description' => 'Ieškokite VU SA posėdžių ir peržiūrėkite atstovavimo rezultatus',
    'meeting_search_title' => 'Posėdžiai',
    'meeting_search_description' => 'Ieškokite VU SA posėdžių ir peržiūrėkite studentų atstovavimo veiklos rezultatus.',
    'search_meetings_placeholder' => 'Ieškoti posėdžių pagal pavadinimą...',
    'filter_meetings' => 'Filtruoti posėdžius',
    'completion_status' => 'Užpildymo būsena',
    'complete' => 'Užpildyti',
    'incomplete' => 'Dalinai užpildyti',
    'no_items' => 'Be darbotvarkės',
    'success_rate' => 'Sėkmingumo rodiklis',
    'success_rate_high' => 'Aukštas (>75%)',
    'success_rate_medium' => 'Vidutinis (50-75%)',
    'success_rate_low' => 'Žemas (<50%)',
    'view_mode_default' => 'Įprasta',
    'view_mode_compact' => 'Kompaktiškas',
    'no_meetings_found' => 'Posėdžių nerasta',
    'enter_search_or_browse_meetings' => 'Įveskite paieškos žodžius arba naršykite visus posėdžius',
    'browse_all_meetings' => 'naršykite visus posėdžius',
    'start_searching_meetings' => 'Pradėkite ieškoti posėdžių',
    'meeting_singular' => 'posėdis',
    'meeting_plural' => 'posėdžiai',

    // Institution/contacts search specific
    'institution_page_title' => 'Kontaktų paieška - VU SR',
    'institution_page_description' => 'Ieškokite VU SA institucijų ir kontaktų',
    'institution_search_title' => 'Kontaktų paieška',
    'institution_search_description' => 'Ieškokite VU SA institucijų ir suraskite visus reikalingus kontaktus vienoje vietoje.',
    'search_institutions_placeholder' => 'Ieškoti institucijų pagal pavadinimą...',
    'institution_filters' => 'Filtruoti institucijas',
    'institution_type' => 'Institucijos tipas',
    'no_types_available' => 'Tipų nerasta',
    'no_institutions_found' => 'Institucijų nerasta',
    'no_institutions_criteria' => 'Pagal jūsų paieškos kriterijus institucijų nerasta.',
    'browse_institutions' => 'Naršyti institucijas',
    'browse_all_institutions' => 'naršykite visas institucijas',
    'enter_search_or_browse_institutions' => 'Įveskite paieškos žodžius arba naršykite visas institucijas',
    'institutions_description' => 'Ieškokite institucijų pagal pavadinimą arba naršykite visas VU SA institucijas.',
    'institution_singular' => 'institucija',
    'institution_plural' => 'institucijos',
    'view_mode_grid' => 'Tinklelis',
    'contact_singular' => 'kontaktas',
    'contacts_plural' => 'kontaktai',

    // Search page navigation
    'go_to_documents' => 'Eiti į dokumentų paiešką',
    'go_to_meetings' => 'Eiti į posėdžių paiešką',
    'go_to_contacts' => 'Eiti į kontaktų paiešką',
];
